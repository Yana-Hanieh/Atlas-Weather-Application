...
function Weather() {
  ...
  const [error, setError] = useState(null); //used for api call error 
  ...
  useEffect (() => { //responsible for fetching the data, it reacts to the location and calls the API Function
    ...
    Promise.all([
      ...
      .catch ((err) => { //promise method runs only if the promise fails/rejectcalls .then() internally, doesnt pass the fulfillment handler, only runs if the error gets passed as the argument
        setError("Failed to load weather data"); //the actual error handling case
      })
  ...
  return (
    <div className="flex flex-row gap-3 px-4 pb-4 items-stretch min-h-screen w-full overflow-y-auto hide-scrollbar"> {/* search bar and side bar are aligned from the top so they start in the same height */}
      ...
      <div className="flex flex-col">
        <p className="h-12"></p>
        <div className=" md:mt-16 w-full h-full lg:w-80 shrink-0 rounded-xl hover:shadow-shadowColor transition-transform hover:-translate-y-2 hover:shadow-sm duration-300">
